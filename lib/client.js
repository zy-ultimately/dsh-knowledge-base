...
window.__ModuleLoader__.load({
  id: '@linxin666/dsh-knowledge-base',
  factory: (require) => {
    var module = { exports: {} };
    ...
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css=' + JSON.stringify("@linxin666/dsh-knowledge-base/KbOverlay.css") + ']') === null) {
        const tag = document.createElement('style');
        tag.dataset.plugin = '@linxin666/dsh-knowledge-base';
        tag.dataset.pluginCss = "@linxin666/dsh-knowledge-base/KbOverlay.css";
        tag.textContent = `
...
`;
    ...
    function ConfigSection({ config, onApply }) {
      const [enabled, setEnabled] = useState(config ? config.enabled !== false : true);
      ...
      useEffect(() => {
        if (config) { setEnabled(config.enabled !== false); setTopK(config.topK); setMinScore(config.minScore); setDir(config.storageDir || ''); }
      }, [config]);
      const apply = () => onApply({ enabled: !!enabled, topK: Number(topK), minScore: Number(minScore), storageDir: dir.trim() });
      return React.createElement('div', { className: 'dsh-kb-cfg' },
        React.createElement('div', { className: 'dsh-kb-cfg-row' },
          React.createElement('label', null, '自动检索（任意会话注入相关片段）'),
          React.createElement('label', { className: 'dsh-kb-toggle' },
        ...
        const timerId = window.setInterval(async () => { 
          try {
            ...
            if (s.parsing === 0) {
              ...
              setDocs(l.docs || []);
              if (l.config) setConfig(l.config);
              // 解析全部完成：清除“正在解析…”提示（若当前提示为解析中）
              setNotice((prev) => (prev && prev.includes('正在解析') ? '' : prev));
            }
          } catch (e) { /* ignore */ }
         }, 1200);
        ...
        open && React.createElement('div', { className: 'dsh-kb-overlay', role: 'presentation' },
          ...
          React.createElement('div', { className: 'dsh-kb-panel', role: 'dialog', 'aria-modal': 'true', 'aria-label': '全局知识库' },
            ...
            React.createElement('div', {
              className: 'dsh-kb-upload' + (busy ? ' busy' : ''),
              onClick: () => inputRef.current && inputRef.current.click(),
            },
              React.createElement('div', null, busy ? '上传中…' : '上传文档'),
